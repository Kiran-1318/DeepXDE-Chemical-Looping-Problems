...
| 3 | Non-isothermal coupled reactor | Coupled `dC/dt` + `dT/dt` | Coupled ODEs, loss weight balancing | DeepXDE + PyTorch both validated vs SciPy RK45 |
...
| 5 | Grain model comparison | `dX/dt = k·(1-X)^(1/3)` | Three-model comparison on same axes | Shrinking core vs grain vs 1st order |
...
| 8 | Inverse PINN: recover A and Ea | Both A and Ea from multi-temperature TGA data | Log-scale variables, per-temperature t_max | **A error: 3.32%, Ea error: 0.25%** |
...
| # | Problem | Physics encoded | Key skill | Result |
|---|---------|----------------|-----------|--------|
| 9 | Multi-cycle redox kinetics | Alternating reduction/oxidation, 3 cycles | Switching ODE with `tf.where`, boundary clustering | MAE: 0.0165, all continuity checks passed |
| 10 | Physics-data hybrid PINN | Shrinking core ODE + sparse observations | Hybrid loss: `loss_physics + λ·loss_data` | Model A: 0.0003 · Model B: 0.34 · Model C: 0.016 |
...
- Always check loss component magnitudes at epoch 0 before tuning weights

**PyTorch (raw implementation):**
- `torch.clamp` on physical T range [100, 1000] K for Arrhenius — NOT on the exponent
- `t.grad.zero_()` inside training loop prevents gradient accumulation across epochs
- IC weight must be proportional to PDE loss magnitude at epoch 0

## Problem 10 as preprint template

Problem 10 maps directly to the Fe₂O₃ ChemRxiv preprint structure:
...
production. The Fe₂O₃ redox PINN targets a ChemRxiv preprint submission
in August 2026.

**Skills built here that transfer directly to the Fe₂O₃ project:**
